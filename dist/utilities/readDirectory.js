...
var _default = function _default(directoryPath) {
  ...
  if (!(0, _validateTargetDirectory.default)(directoryPath, {
  ...
  children = _lodash.default.filter(children, fileName => {
    ...
    if (_lodash.default.startsWith(fileName, 'index.js')) {
      return false;
    }

    if (!isDirectory && !extensions.some(ext => {
      return _lodash.default.endsWith(fileName, '.' + ext);
    })) {
      return false;
    }

    if (isDirectory && (!(0, _hasIndex.default)(absolutePath) || ignoreDirectories)) {
      return false;
    }
